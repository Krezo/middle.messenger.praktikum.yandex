...
abstract class Block<Props extends { [key: string]: unknown } = any> {
  ...
  props: Props;
  _element: HTMLElement | Text | SVGSVGElement | SVGLineElement | SVGPolygonElement;
  _meta: { tagName: keyof HTMLElementTagNameMap, props: { [K in keyof Props]?: Props[K] } };

  constructor(tagName: keyof HTMLElementTagNameMap = "div", props = {} as Props) {
  ...
  abstract render(): string | HTMLElement | Text | SVGSVGElement | SVGLineElement | SVGPolygonElement
      ...
      set(target, prop, newValue) {
        ...
        if (oldPropValue !== 'newValue') {
          ...
          self.eventBus().emit('flow:component-did-update',
            { ...self.props, [prop]: oldPropValue }, { ...self.props, [prop]: newValue })
        }
        return true;
      }
    })
